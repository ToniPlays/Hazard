...
namespace HazardRenderer 
{
    ...
    Ref<Shader> Shader::Create(const std::string& debugName, const std::unordered_map<uint32_t, std::string>& shaderModules)
    {
		...
		case RenderAPI::Vulkan: return Ref<Vulkan::VulkanShader>::Create(debugName, shaderModules);
#endif
