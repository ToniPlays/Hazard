...
namespace Hazard::Rendering::Vulkan
{
	...
	void SwapChain::CreateSwapChain()
	{
		...
		m_SurfaceFormat = VKUtils::ChooseSwapChainFormat(details.formats, VK_FORMAT_A8B8G8R8_SSCALED_PACK32);
		...
		for (size_t i = 0; i < m_ImageCount; i++)
		{
			...
			std::vector<FrameBufferAttachment> attachments(2);
			attachments[0].userData = m_SwapChainImageViews[i];
			attachments[1].userData = m_RenderPass->GetDepthImage(i);

			FrameBufferCreateInfo frameBufferCreateInfo = {};
			frameBufferCreateInfo.Width = m_Extent.width;
			frameBufferCreateInfo.Height = m_Extent.height;
			frameBufferCreateInfo.userData = m_RenderPass.get();
			frameBufferCreateInfo.Attachments = attachments;
			frameBufferCreateInfo.AttachmentCount = 2;

			m_SwapChainFrameBuffers[i] = new VulkanFrameBuffer(&frameBufferCreateInfo);
		}
