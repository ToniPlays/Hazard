...
namespace UI
{
	GameViewport::GameViewport() : Panel("Game Viewport", false)
	{
        return;
	...
	void GameViewport::Update()
	{
		...
		cameraData.Position = tc->GetTranslation();
	...
	void GameViewport::OnPanelRender()
	{
        ...
        else ImUI::Image(m_FrameBuffer->GetImage(), RenderContextManager::GetDefaultSampler(), size);
	}
