...
namespace ImGui 
{
	bool TreeNodeWithIcon(Ref<Hazard::Rendering::Texture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint/* = IM_COL32_WHITE*/)
	{
		...
		const ImU32 arrow_col = Style::GetStyleColor32(selected ? ColorType::Background : ColorType::Secondary);
		...
		if (display_frame)
		{
			...
			if (icon)
			{
				...
				WindowLayout::Layout::ShiftY(-frame_height + pad);
				WindowLayout::Layout::ShiftX(arrowWidth);
