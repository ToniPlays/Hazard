...
void main() 
{
	...
	vec4 origin = u_Camera.ViewInverse * vec4(0.0, 0.0, 0.0, 1.0);
	vec4 target = u_Camera.ProjInverse * vec4(d.x, d.y, 1.0, 1.0);

	vec4 direction = u_Camera.ViewInverse * vec4(normalize(target.xyx), 0.0);
